...
def create_radar(data_df, cat_name, virtues_dict):
    ...
    fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[1, 5])), title=f"이번 달 {cat_name} 세부 분석", height=450)
    return fig
    ...
    if menu == "🌱 학생 기록 및 조회":
        ...
        with t1:
            ...
            if st.button("🚀 기록 저장하기"):
                ...
                row += [reflection, ""] # 선생님 피드백 칸은 비워서 저장
                ws.append_row(row)
        ...
        # (선생님 페이지 로직 - 지난번 드린 대영역 평균 요약 버전 포함)
        st.title("🔐 관리자 대시보드")
        ...
        if pw == "bh1123":
            ...
            if all_data:
                ...
                def get_summary(in_df):
                    return [in_df[[f'성장{i}' for i in range(1,6)]].mean().mean(), 
                            in_df[[f'공감{i}' for i in range(1,6)]].mean().mean(), 
                            in_df[[f'행복{i}' for i in range(1,6)]].mean().mean()]
                
                cats = ['성장 평균', '공감 평균', '행복 평균']
                with col1:
                    st.plotly_chart(px.bar(x=cats, y=get_summary(full_df), title="학년 전체 평균", range_y=[1,5]), use_container_width=True)
                with col2:
                    st.plotly_chart(px.bar(x=cats, y=get_summary(class_df), title="학급 평균", range_y=[1,5], color_discrete_sequence=['orange']), use_container_width=True)

                # 학생 개별 피드백 관리 부분
                st.divider()
                st.subheader("✍️ 학생별 피드백 남기기")
                if not class_df.empty:
                    ...
                    for i, row in s_data.iterrows():
                        with st.expander(f"📅 {row['시간'].strftime('%m/%d')} 기록 (다짐: {row['반성의글'][:20]}...)"):
                            st.write(f"**전체 다짐:** {row['반성의글']}")
                            fb_input = st.text_input("선생님 피드백", value=str(row['선생님피드백']), key=f"fb_{i}")
                            if st.button("저장", key=f"btn_{i}"):
                                ws_up = sheet.worksheet(f"{grade_sel}학년{class_sel}반")
                                ws_up.update_cell(int(row['sheet_row']), 21, fb_input) # 21번째 열이 피드백 칸
